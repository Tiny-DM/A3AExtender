...
if (!isNull _veh && {!(_veh isKindOf "StaticWeapon")}) then {
    if !(isNil "TNY_disableUnitAdding") exitWith {};
    diag_log format ["Running spectator func for %1", typeOf _veh];
    private _spectators = [] call ace_spectator_fnc_players;
    ["ADDUNIT", [_unit]] remoteExec ["A3AE_ONE_LIFE_FUNCTIONS_fnc_manageSpectator", _spectators]; 
};
