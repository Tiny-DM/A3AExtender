...
switch (_mode) do
{
    case ("onLoad"):
    {
        ...
        ["update"] call A3A_GUI_fnc_recruitSquadDialog;
    ...
    case ("update"):
    {
...

        {
            ...
            if (_group isEqualTo [] || {(_x#0) isEqualTo A3A_IDC_RECRUITMORTARTEAMICON}) then {
                _button ctrlEnable false;
                if ((_x#0) isEqualTo A3A_IDC_RECRUITMORTARTEAMICON) then {
                    _button ctrlSetTooltip "Buying mortar teams is disabled in hardcore";
                } else {
