...
Example:
    ["update"] spawn A3A_GUI_fnc_playerTab);
...
switch (_mode) do
{
    case ("update"):
    {
        ...
        if (_isUndercover) then {
            ...
            // _undercoverButton ctrlAddEventHandler ["MouseButtonClick", {player setCaptive false; ["update"] spawn A3A_GUI_fnc_playerTab)}];
            // _undercoverIcon ctrlSetTextColor ([A3A_COLOR_WHITE] call A3A_GUI_fnc_configColorToArray));
            ...
            _undercoverIcon ctrlSetTextColor ([A3A_COLOR_BUTTON_BACKGROUND_DISABLED] call A3A_GUI_fnc_configColorToArray);
        ...
        if (_fastTravelBlockers isEqualTo []) then {
            ...
            _fastTravelIcon ctrlSetTextColor ([A3A_COLOR_WHITE] call A3A_GUI_fnc_configColorToArray);
            ...
            _fastTravelIcon ctrlSetTextColor ([A3A_COLOR_BUTTON_BACKGROUND_DISABLED] call A3A_GUI_fnc_configColorToArray);
        ...
        if (_canBuild # 0) then
        {
            ...
            _constructIcon ctrlSetTextColor ([A3A_COLOR_WHITE] call A3A_GUI_fnc_configColorToArray));
            _constructIcon ctrlSetTooltip "";
        } else {
            _constructButton ctrlEnable false;
            _constructButton ctrlSetTooltip (_canBuild # 1);
            _constructIcon ctrlSetTextColor ([A3A_COLOR_BUTTON_BACKGROUND_DISABLED] call A3A_GUI_fnc_configColorToArray));
        ...
        */
        ...
        if (_canManageAi) then {
            ...
            _aiManagementIcon ctrlSetTextColor ([A3A_COLOR_WHITE] call A3A_GUI_fnc_configColorToArray);
        } else {
            _aiManagementButton ctrlEnable false;
            _aiManagementButton ctrlSetTooltip _aiManagementTooltipText;
            _aiManagementIcon ctrlSetTextColor ([A3A_COLOR_BUTTON_BACKGROUND_DISABLED] call A3A_GUI_fnc_configColorToArray);
        };
        ...
        private _timeRemainingText = if (A3A_softBannedUIDList findIf {_x#0 == getPlayerUID player} > -1) then {
            private _timeRemaining = [player] call A3AE_ONE_LIFE_FUNCTIONS_fnc_getBanTimeRemaining;
            format ["Time until revive: %1", _timeRemaining];
        } else {
            name player;
        };

        _playerNameText ctrlSetText _timeRemainingText;
        ...
        if (theBoss == player) then {
            ...
            _commanderPicture ctrlSetTextColor ([A3A_COLOR_COMMANDER] call A3A_GUI_fnc_configColorToArray);
            // Update text
            _commanderText ctrlSetText localize "STR_antistasi_dialogs_main_commander_text_commander";
            _commanderText ctrlSetTextColor ([A3A_COLOR_COMMANDER] call A3A_GUI_fnc_configColorToArray);
            // Update button
            ...
            if (player getVariable ["eligible", false]) then {
                ...
                _commanderPicture ctrlSetTextColor ([A3A_COLOR_ELIGIBLE] call A3A_GUI_fnc_configColorToArray);
                // Update text
                _commanderText ctrlSetText localize "STR_antistasi_dialogs_main_commander_text_eligible";
                _commanderText ctrlSetTextColor ([A3A_COLOR_ELIGIBLE] call A3A_GUI_fnc_configColorToArray);
                // Update button
                ...
                _commanderPicture ctrlSetTextColor ([A3A_COLOR_INELIGIBLE] call A3A_GUI_fnc_configColorToArray);
                // Update text
                _commanderText ctrlSetText localize "STR_antistasi_dialogs_main_commander_text_ineligible";
                _commanderText ctrlSetTextColor ([A3A_COLOR_INELIGIBLE] call A3A_GUI_fnc_configColorToArray);
                // Update button
