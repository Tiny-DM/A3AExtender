...
switch (_mode) do
{
    case ("onLoad"):
    {
		["openInfo"] call A3A_GUI_fnc_roleSelectDialog;
    };
	case ("update"):
	{
		{
			...
			private _currentCount = {_x getVariable ["A3A_Role", "none"] == _roleName} count (units group player select {isPlayer _x});
			private _maxCount = [_roleName] call A3A_fnc_getRoleCap;
			if (_currentCount < _maxCount || {_roleName == "rifleman" || {_roleName == "commander" && {isNull theBoss}}}) then {
				_icon ctrlSetTextColor ([A3A_COLOR_WHITE] call A3A_GUI_fnc_configColorToArray);
            	_icon ctrlSetTooltip "";
			} else {
				_icon ctrlSetTextColor ([A3A_COLOR_BUTTON_BACKGROUND_DISABLED] call A3A_GUI_fnc_configColorToArray);
    ...
    case ("openRole"):
    {
		...
		private _currentPlayers = units group player select {_x getVariable ["A3A_Role", "none"] == _roleName};
	...
	case ("selectRole"):
	{
		...
		["openRole",[_roleName]] call A3A_GUI_fnc_roleSelectDialog;
		...
		[_roleName] call A3A_fnc_unitTraits;
		closeDialog 0;
	};
	case ("closeDialog"):
	{
		...
		if (player getVariable ["A3A_Role","none"] == "none") then {
			["rifleman"] call A3A_fnc_unitTraits;
		};
		closeDialog 0;
	};
};
