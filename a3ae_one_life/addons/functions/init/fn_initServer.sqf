...
addMissionEventHandler ["GroupCreated", {
    ...
    _group addEventHandler ["UnitLeft", {
        ...
        if (isPlayer _oldUnit) then {
            private _currentRole = _oldUnit getVariable ["A3A_Role", "rifleman"];
            if (_currentRole in ["rifleman", "commander"]) exitWith {};
            _oldUnit setVariable ["A3A_Role", "rifleman", true];
            ["rifleman",true] remoteExecCall ["A3A_fnc_unitTraits", _oldUnit];
            ["Role Selection", "You left your group so you have been reverted to a rifleman."] remoteExecCall ["A3A_fnc_customHint", _oldUnit];
        };
    }];
}];
