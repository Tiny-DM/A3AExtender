...
{
    if (toUpper _supportType isEqualTo "AC119BOMB") exitWith {
        _text = format ["%1 is loading a C-119 with a big-ass bomb.", _sideName];
    };
    if (toUpper _supportType isEqualTo "AC119ORBIT") exitWith {
        _text = format ["%1 is preparing an AC-119 gunship.", _sideName];
    };
