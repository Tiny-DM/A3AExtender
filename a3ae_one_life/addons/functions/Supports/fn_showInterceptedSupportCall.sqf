...
{
    if (toUpper _supportType in ["AC119BOMB", "AC119ORBIT"]) then {
        if (toUpper _supportType isEqualTo "AC119BOMB") then {
            _text = format ["A %1 C-119 is creating a new LZ.", _sideName];
            _markerText = "Tactical demolition area";
        } else {
            _text = format ["A %1 AC-119 gunship knows where you are.", _sideName];
            _markerText = "Area of regret";
        };
    } else {
