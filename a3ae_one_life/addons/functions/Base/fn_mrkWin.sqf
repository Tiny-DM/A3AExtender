...
_result = if ((count (airportsX select {sidesX getVariable [_x,sideUnknown] == teamPlayer}) == 2) && {_markerX in airportsX && {A3A_funnyEnding}}) then {
    if (isNil "A3AE_playerFlagAttempt") then {A3AE_playerFlagAttempt = 0};
    A3AE_playerFlagAttempt = A3AE_playerFlagAttempt + 1;
    _message = switch (A3AE_playerFlagAttempt) do {
        case (1): {"Are you sure you want to end the campaign, here and now?<br/><br/><br/>Try again if you're sure."};
        case (2): {"I mean come on, is it really worth it? YOU are ending it for EVERYONE. How does it feel?<br/><br/><br/>Try again if you're really sure."};
        case (3): {"Alright, but don't tell me I didn't warn you...<br/><br/><br/>The campaign will have a specific, cinematic ending. It is recommended to wait until you have a good number of players online or an admin says you're good to flip.<br/><br/>The next usage will actually flip the flag."};
        default {"Thanks for playing. I put a ton of work into this next sequence, so I hope you appreciate it.<br/><br/><br/>- Tiny"};
    };
    ["War Winning", _message] call A3A_fnc_customHint;
    
    if !(A3AE_playerFlagAttempt > 3) exitWith {false};
    true;
} else {true;};
if (!_result) exitWith {};
