...
_result = if ((count (airportsX select {sidesX getVariable [_x,sideUnknown] == teamPlayer}) == 2) && {_markerX in airportsX && {A3A_funnyEnding}}) then {
    ...
    if (isNil "A3AE_finalFlipReq") then {A3AE_finalFlipReq = 10;};
    ...
    _message = switch (A3AE_playerFlagAttempt) do {
        ...
        case (3): {"Alright, but don't tell me I didn't warn you...<br/><br/><br/>The flag concedes. Flip for real this time, if you dare."};
    ...
    missionNamespace setVariable ["A3A_oneLifeFlagGarageBlock",true,true];
    true;
} else {true;};
if (!_result) exitWith {};
