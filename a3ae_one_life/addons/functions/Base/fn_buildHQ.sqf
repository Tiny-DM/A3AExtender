...
if ((count _friendlyAirports == 2) && {(petros distance2D (getMarkerPos (_enemyAirports#0))) < 4000 && {A3A_funnyEnding}}) then {
	...
	["War Winning", "Try to avoid hitting the last airbase until the whole map is captured. If you do it anyways, try to make it a joint AO with the whole server."] remoteExec ["A3A_fnc_customHint",2];
};
