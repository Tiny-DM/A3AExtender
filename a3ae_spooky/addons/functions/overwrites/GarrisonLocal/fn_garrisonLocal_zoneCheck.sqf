...
if (_type in ["camp", "roadblock", "rebpost"]) exitWith
{
    ...
    } forEach (_troops + (_garrison get "vehicles"));
...
private _defenders = (units _side inAreaArray _marker) select {!(_x getVariable ["A3A_isZombie", false])};
_defenders insert [-1, (units _side inAreaArray [_markerPos, 100, 100]) select {!(_x getVariable ["A3A_isZombie", false])}, true];
