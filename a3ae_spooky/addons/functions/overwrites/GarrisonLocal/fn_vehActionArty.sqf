...
private _isGas = if (missionNamespace getVariable ["A3A_nextMortarGas", false]) then {
    missionNamespace setVariable ["A3A_nextMortarGas", false, true];
    true;
} else {
    false;
};
...
if (_isGas) then {_spreadOffset = 200};
...
// Ranging shots
private _subTargets = [];
private _rangingShots = if (_mortar distance2d _targetPos < 1500 + random 1500) then {
...
_mortar setVariable ["A3A_rangingShots", _rangingShots];
...

{
    ...
    private _remRangingShots = _mortar getVariable ["A3A_rangingShots", 0];
    if (_isGas && (_remRangingShots < 1)) then {
        _mortar setVariable ["A3AE_spooky_firingGas", true];
    } else {
        _mortar setVariable ["A3A_rangingShots", _remRangingShots - 1];
    };
    sleep _interval;
